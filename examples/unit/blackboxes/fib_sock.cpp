...
void fib_lfsr::setup() {
    ...
    if (!child_pid) {
        ...
        m_signal_io.recv();
        if (child_pid == m_signal_io.get<int>(fib_ports::__pid__)) {
...
void fib_lfsr::handle_event(SST::Event *ev) {
    ...
    if (se) {
        ...
        m_signal_io.set(fib_ports::reset, std::stoi(_data_in.substr(2, 1)));
        m_signal_io.set(fib_ports::__clock__, std::stoi(_data_in.substr(3)));
        ...
        std::string _data_out = std::to_string(m_signal_io.get<int>(fib_ports::data_out));
