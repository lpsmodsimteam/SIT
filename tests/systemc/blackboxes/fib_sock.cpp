...
fib_lfsr::fib_lfsr(SST::ComponentId_t id, SST::Params &params)
    : SST::Component(id), m_signal_io(FIB_NUM_PORTS, socket(AF_UNIX, SOCK_STREAM, 0)),
...
void fib_lfsr::setup() {
    ...
    if (!child_pid) {
        ...
        m_signal_io.recv();
        if (child_pid == m_signal_io.get<int>(fblfsr_ports.pid)) {
...
void fib_lfsr::handle_event(SST::Event *ev) {
    ...
    if (se) {
        ...
        m_signal_io.set(fblfsr_ports.reset, std::stoi(_data_in.substr(2, 1)));
        m_signal_io.set(fblfsr_ports.clock, std::stoi(_data_in.substr(3)));
        ...
        std::string _data_out = std::to_string(m_signal_io.get<int>(fblfsr_ports.data_out));
