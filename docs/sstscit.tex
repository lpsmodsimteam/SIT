...
\end{lstlisting}
  ...
  A simulation model has been developed to demonstrate the project. The model simulates a traffic intersection controlled by two traffic lights. A flow of traffic is simulated through a road only when its traffic light generates a green or yellow light with the other generating a red light.

  The concept is derived from <INCLUDE REFERENCE HERE>.

    \subsection{SST Components}
    The simulation project consists of three SST components: \lstinline{car_generator}, \lstinline{traffic_light} and \lstinline{intersection}.  All the components with the exception of \lstinline{traffic_light} were inherited from the original project.

      \subsubsection{car\_generator} A component 
